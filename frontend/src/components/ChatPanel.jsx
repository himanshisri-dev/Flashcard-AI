    ...
    <section className="mt-10">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-slate-900 dark:text-white font-semibold">Ask your document</h2>
            <p className="text-xs text-slate-400 dark:text-slate-500">Powered by RAG + Groq</p>
          </div>
        </div>
        {messages.length > 0 && (
          <button
            onClick={handleClear}
            className="text-xs text-slate-400 dark:text-slate-600 hover:text-rose-500 dark:hover:text-rose-400 transition-colors"
          >
            Clear history
          </button>
        )}
      ...
      {/* Not ready state */}
      {!ready && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/8 rounded-2xl p-6 text-center">
          {embeddingStatus === "pending" ? (
            <div className="flex flex-col items-center gap-3">
              <svg className="w-7 h-7 animate-spin text-amber-500 dark:text-amber-400" viewBox="0 0 24 24" fill="none">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
              </svg>
              <p className="text-amber-600 dark:text-amber-400 text-sm font-medium">Preparing your document for Q&A…</p>
              <p className="text-slate-400 dark:text-slate-500 text-xs">This takes about a minute on first upload.</p>
            </div>
          ) : embeddingStatus === "failed" ? (
            <p className="text-rose-500 dark:text-rose-400 text-sm">Q&A index failed. Use Reindex on the dashboard.</p>
          ) : (
            <p className="text-slate-400 dark:text-slate-500 text-sm">Q&A not available for this document.</p>
          )}
        </div>
      )}
      ...
      {ready && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/8 rounded-2xl overflow-hidden">
          ...
          <div className="h-[420px] overflow-y-auto p-5 space-y-4">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                ...
                <div>
                  <p className="text-slate-700 dark:text-slate-300 text-sm font-medium">Ask anything about this document</p>
                  <p className="text-slate-400 dark:text-slate-600 text-xs mt-1">The AI will answer using only the document content.</p>
                </div>
              </div>
            )}
            ...
            {messages.map((m) => (
              <div key={m._id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"} gap-2.5`}>
                ...
                {m.role === "user" && (
                  <div className="w-7 h-7 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center flex-shrink-0 mt-0.5 text-slate-600 dark:text-slate-300 text-xs font-bold">
                    U
                  </div>
                )}
              </div>
          ...
          {/* Input bar */}
          <div className="border-t border-slate-200 dark:border-white/8 p-4">
            {error && (
              <p className="text-xs text-rose-500 dark:text-rose-400 mb-3 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 rounded-xl px-3 py-2">
                {error}
              </p>
            )}
            <form onSubmit={handleSend} className="flex gap-2">
              <input
                ...
                disabled={streaming}
                className="flex-1 px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm disabled:opacity-50"
              />
              <button
                type="submit"
